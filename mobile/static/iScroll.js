;(function() {
  ...
  const vendor = /webkit/i.test(navigator.appVersion)
    ? 'webkit'
  ...
  const isIDevice = /iphone|ipad/gi.test(navigator.appVersion)
  const isPlaybook = /playbook/gi.test(navigator.appVersion)
  ...
  const hasTransform = vendor + 'Transform' in document.documentElement.style
  const hasTransitionEnd = isIDevice || isPlaybook
  ...
  const START_EV = hasTouch ? 'touchstart' : 'mousedown'
  const MOVE_EV = hasTouch ? 'touchmove' : 'mousemove'
  const END_EV = hasTouch ? 'touchend' : 'mouseup'
  const CANCEL_EV = hasTouch ? 'touchcancel' : 'mouseup'
  const WHEEL_EV = vendor == 'Moz' ? 'DOMMouseScroll' : 'mousewheel'
  let trnOpen = 'translate' + (has3d ? '3d(' : '(')
  let trnClose = has3d ? ',0)' : ')'
  const iScroll = function(el, options) {
    const that = this
    const doc = document
    let i
  ...
  iScroll.prototype = {
    ...
    _move: function(e) {
      const that = this
      const point = hasTouch ? e.touches[0] : e
      let deltaX = point.pageX - that.pointX
      let deltaY = point.pageY - that.pointY
      let newX = that.x + deltaX
      let newY = that.y + deltaY
      let c1
      let c2
      let scale
    ...
    _end: function(e) {
      ...
      const duration = (e.timeStamp || Date.now()) - that.startTime
      let newPosX = that.x
      let newPosY = that.y
      let distX
      let distY
    ...
    refresh: function() {
      ...
      if (typeof that.options.snap === 'string') {
        ...
        for (i = 0, l = els.length; i < l; i++) {
          ...
          that.pagesX[i] =
            pos.left < that.maxScrollX ? that.maxScrollX : pos.left * that.scale
          that.pagesY[i] =
    ...
    zoom: function(x, y, scale, time) {
      const that = this
      const relScale = scale / that.scale
      if (!that.options.useTransform) {
        return
      }
      that.zoomed = true
