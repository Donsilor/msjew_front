  ...
  if (process.server && isWap(u)) {
    ...
    const toWapUrl = path => {
      ...
      let rules = [
        ...
        // 订婚
        {
          'pcUrl':/^\/engagementRingDetails$/,
          'mobileUrl':`/engagement`
        },
        // 结婚
        {
          'pcUrl':/^\/weddingRingDetails$/,
          'mobileUrl':`/marriage-ring`
        },
        // 钻石
        {
          'pcUrl':/^\/seleceDiamond\/(.*)\/2$/,
          'mobileUrl':`/diamond/diamonds?goodId=${path.match(diamondsReg)[1]}`
        },
        // 珠宝饰品
        {
          'pcUrl':/^\/seleceJewellery\/(.*)\/1$/,
          'mobileUrl':`/diamond/diamonds?goodId=${path.match(diamondsReg)[1]}`
        },
        // 结婚单戒
        {
          'pcUrl':/^\/seleceWedding\/(.*)$/,
          'mobileUrl':`/marriage-ring/single-ring-detail?goodId=${
            path.match(singleRingReg)[1]
          }`
        },
        // 结婚对戒
        {
          'pcUrl':/^\/seleceRightRing\/(.*)$/,
          'mobileUrl':`/marriage-ring/pair-ring-detail?ringId=${path.match(pairRingReg)[1]}`
        },
        // 结婚对戒
        {
          'pcUrl':/^\/seleceEngagement\/(.*)\/2$/,
          'mobileUrl':`/engagement-rings/engagement-rings?goodId=${
