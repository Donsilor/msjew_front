  ...
  <div class="page-cover" v-loading="loading">
    <div class="page-content">
      <section class="search-condition">
        ...
        <!-- 系列 --> 
        <div style="padding-left: 137px;" class="condition-item condition-style condition-lady-style ">
          <h2 class="condition-name">
            {{ $t(`${lang}.series`) }}
          </h2>
          <ul class="options">
            <li
              v-for="(option, index) in designerOptions"
              :key="index"
              :class="[
                'option-item',
                { active: option.id === searchConditions.series }
              ]"
              @click="changeSeries(option.id)"
            >
              <div class="item-icon">
                <img :src="option.image" />
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      designerOptions: this.CONDITION_INFO.style.designer,
