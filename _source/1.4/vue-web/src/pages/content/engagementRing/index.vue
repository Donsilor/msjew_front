<template>
	<div class="engagement-accessories">
		<Header :title="lang.title" icon="&#xe621;" typeId="2" v-if="cusomIndex>-1"></Header>
		<div class="engagement-content">
			<div class="engagement-top clearfix">
				<div class="search">
					<label class="title">{{ lang.style }}</label>
					<ul>
						<li v-for="(item, index) in lang.engagementRing[1].list" :class="{active:wemenIndex == item.id}" @click="wemenSearch(item.id)">
				...
				<div class="material">
					...
					<ul>
						...
						<li v-for="(item, index) in lang.engagementRing[2].list" :class="{active:materialIndex == item.id}" @click="changeMaterial(item.id)" v-if="index<6">
			...
			<div class="m-title">
				...
				<p class="font-size-12 margin-bottom-30" v-if="wemenIndex==-1">{{ lang.defaultDesc }}</p>
				<p class="font-size-12 margin-bottom-30" v-for="(item, index) in lang.engagementRing[1].list" v-if="wemenIndex==item.id">{{item.desc}}</p>
			</div>
			<div class="engagement-list" v-loading="loading">
				<div class="sort">
...
<script>
	...
	export default {
		...
		data() {
			return {
				lang: this.$LANGUAGE.engagementRing.index,
				loading: true,
		...
		watch: {
			$route: {
				handler: function(val, oldVal) {
					if(val.name != 'seleceEngagement' && val.name != 'custom') {
						this.init();
					}
				},
			},
