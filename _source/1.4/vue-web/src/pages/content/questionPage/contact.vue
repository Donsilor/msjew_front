      ...
      <dl class="info">
        ...
        <dd class="item">
          ...
          <div class="dd-content">
            ...
            <p class="dd-desc">
              <!-- {{ lang.addressDetail }} -->
              深圳市罗湖区东晓街道独树社区步心路3008号IBC商务珠宝大厦A座1306
            </p>
          </div>
        </dd>
      </dl>
      ...
      <div class="reservation-content">
        ...
        <div class="row-line">
          <div class="select country-code">
            <div :class="['select-text', { active: checkedPhoneCodeInfo.value }]">
    ...
    <section
      ...
      <div class="successful-content">
        <div class="status-icon">
          <img src="/static/images/knowledge/contact-successful.png" >
        </div>
...
<script>
  import {postJsonRequest} from '@/api/user-axios';
import PhoneJson from '@/assets/countrycode.json';
import Moment from 'moment';
export default {
  data() {
    return {
      lang: this.$LANGUAGE.questionPage.contact,
      ringTypes: [
        {
          type: 2,
          name: this.$LANGUAGE.questionPage.contact.engagementRing
        },
        {
          type: 1,
          name: this.$LANGUAGE.questionPage.contact.marriageRing
        },
        {
          type: 3,
          name: this.$LANGUAGE.questionPage.contact.fashionAccessory
        }
      ],
      bookTimeOptions: [
        {
  ...
  methods: {
    ...
    submit() {
      ...
      if (!info.lastName) {
        this.$message.error(this.lang.inputTheLastName);
        return;
      }

      if (!info.firstName) {
        this.$message.error(this.lang.inputTheFirstName);
        return;
      }

      if (!info.telphone) {
        this.$message.error(this.lang.inputTelphone)
        return;
      }

      if (!info.bookDate) {
        this.$message.error(this.lang.selectTheCorrectDate);
        return;
      }

      if (!info.bookTime) {
        this.$message.error(this.lang.selectAnAppointmentTime);
        return;
      }
      ...
      postJsonRequest(`/web/reservation/addReservation`, info)
        .then(res => {
