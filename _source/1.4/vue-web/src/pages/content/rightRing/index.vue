		...
		<div class="wedding-content">
			<div class="wedding-top clearfix">
				<div class="search">
					...
					<ul>
						<li v-for="(item, index) in weddingRing[0].list" :class="{active:styleValue == item.id}" @click="styleSearch(item.id)">
				...
				<div class="material">
					...
					<ul>
						<li :class="{active:materialValue == 0}" @click="changeMaterial(0)">
							<i><img src="../../../../static/images/jewelleryAccessories/material-all.png" /></i>
							<span>{{ lang.materials }}</span>
						</li>
						<li v-for="(item, index) in weddingRing[3].list" :class="{active:materialValue == item.id}" @click="changeMaterial(item.id)" v-if="index<6">
			...
			<div class="m-title">
				...
				<p class="font-size-12">{{ lang.defaultDesc  }}</p>
			</div>
			<div class="wedding-list" v-loading="loading">
				...
				<ul class="list-body" >
					<li class="li" v-for="(item, index) in goodsList" v-if="item.salePrice>0"  @click="goSelece(item.id,$event)" @mouseenter="productMouseEnter" @mouseleave="productMouseLeave">
						<div>
							<img class="img1"  v-if="item.ringImg && item.ringImg.split(',').length>=3" :src="IMG_URL+item.ringImg.split(',')[2]" />
							<img class="img2" :src="IMG_URL+item.ringImg.split(',')[0]" :alt="item.goodsName" />
...
<script>
	...
	export default {
		...
		data() {
			return {
				lang: this.$LANGUAGE.rightRing.index,
		...
		methods: {
			...
			init() {
				...
				if(this.paramsId!=id){
					this.paramsId=id;
					this.index = -1;
					this.styleValue = this.$route.params.id;
			...
			searchGoods(type) {
				...
				this.paramsData.beginPrice = this.removeFormatMoney(this.priceMin);
				this.paramsData.endPrice = this.removeFormatMoney(this.priceMax);
				this.paramsData.styleValue = parseInt(this.styleValue);
				this.paramsData.materialValue = this.materialValue;
				...
				this.$postJsonRequest(`/web/ring/searchRing`, this.paramsData).then(res => {
					this.loading=false;
					if(type != 1) { //清空列表
						this.goodsList = [];
					}
