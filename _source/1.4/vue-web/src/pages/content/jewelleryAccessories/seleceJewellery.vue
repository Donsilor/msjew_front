...
<template>
	<div class="selece-engagement">
		<Header v-if="goods.goodsMod==1" :title='title' :icon="icon" typeId="3" @refresh="infoCustom()"></Header>
		<div class="d-header clearfix">
			...
			<div class="r-title">
				...
				<b class="price">{{goods.coinType}} {{sizeIndex==-1 && goods.sizes ?formatMoney(goods.salePrice):(detail?formatMoney(detail.retailMallPrice):'0.00')}}</b>
				<div class="btn" v-if="goods.goodsStatus==2">
					<a v-if="goods.goodsMod==1" class="add-card" :class="!isAdd?'no-card':''" @click="addMore">{{ lang.confirmSelection }}</a>
					<a v-if="!step1" class="add-card" :class="[!isAdd?'no-card':'', goods.goodsMod==1?'add-card-more':'']" @click="addCardList">{{ lang.addCard }}</a>
				</div>
				<div class="btn" v-if="goods.goodsStatus!=2">
...
<script>
	...
	import { setCartList, getCartList, getCustom, clearCustom, setCustom, setTwoCustom, getTwoCustom, clearTwoCustom } from '@/api/cache.js';
	...
	export default {
		...
		data() {
			return {
				lang: this.$LANGUAGE.jewelleryAccessories.seleceJewellery,
				anchorPoint: 0, //锚点索引
				imgIndex: 0, //图片索引
				pullIcon: true,
				materialIndex: 0,
		...
		methods: {
			...
			infoCustom() {
				...
				if(this.$route.params.key !== 'look1' && this.$route.params.key !== 'look2') {
					...
					if(firstInfo && firstInfo.categoryId > 2) {
						clearCustom()
						firstInfo = getCustom()
					}
				}
			...
			searchDetail() {
				...
				if(this.goods.details) {
					if(this.goods.sizes) {
						...
						for(var i = 0, len = this.goods.details.length; i < len; i++) {
							var detail = this.goods.details[i];
							if(detail.material == this.goods.materials[this.materialIndex].id) {
								this.detail = detail;
								if(this.detail.stock > 0) {
									this.isAdd = true;
								}
			...
			addMore() {
				if(this.isAdd) {
					//先判断是否有相同的产品并且第二步已选择的情况下，认为是修改，然后跳到定制第三步，
					//再判断是否有相同的产品并且第二部没有选择的情况下，认为是修改，但是此时跳到第二步
					if(this.firstdata && this.detail.categoryId == this.firstdata.categoryId) {
						setCustom(this.detail)
						if(this.step2) {
							setCustom(this.detail)
							this.$router.push({
								name: "custom",
					...
					} else if(this.twodata && this.detail.categoryId == this.twodata.categoryId) {
						setTwoCustom(this.detail)
						this.$router.push({
							name: "custom",
							params: {
								id: this.detail.goodsId
					...
					} else { //如果有第一步（对戒列表已经判断了）第一步一定是鑽石
						if(!this.step1) {
							setCustom(this.detail)
							this.$router.push({
								name: 'diamond',
