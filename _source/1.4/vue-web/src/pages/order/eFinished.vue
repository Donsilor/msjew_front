  ...
  <div
    ...
    <div class="order-step">
      ...
      <div class="one-step">
        ...
        <div class="step-title">{{ lang.finishePay }}</div>
        <div class="step-desc">{{ lang.balabalabala }}</div>
      </div>
      <div class="step-line" />
    </div>
    <div
      :style="[{'width': getTokens ? '100%' : '50%'}]"
      class="success-info pull-left">
      <div class="diamond-img">
        <img
          src="../../../static/images/order/diamond.png"
          alt="">
        <div><i class="iconfont icongou"/></div>
      </div>
      <div class="order-email">
        <span>{{ lang['ewe-will-send-to'] }}</span><span>{{ data.afterMail }}</span><i class="iconfont iconzuoshangjiantou"/>
      </div>
      <div class="order-send">{{ lang.edaysGone }}</div>
      <div class="order-num">{{ lang.orderCode }}<span>{{ data.orderNo }}</span></div>
      <div
        class="order-guide"
        @click="showEftGuide">{{ lang.eGuide }}</div>
      <eftGuide ref="eft-guide"/>
    </div>
    <div
      v-if="!getTokens"
      class="success-login pull-left">
      <h4>{{ lang.registered }}</h4>
      <p>{{ lang.balabalabalaAgain }}</p>
      <span @click="gologin">{{ lang.registered }}</span>
    </div>
    <div class="middle-bar">
      <i class="iconfont icongantanhao"/><span>{{ lang.balabalaAgain }}</span>
    </div>
    <div class="order-info">
      <div class="left-info">
        <div class="address-info">
          ...
          <div class="user-info">
            <div class="user-name">{{ data.orderAddress.lastName }}{{ data.orderAddress.firstName }}<span>{{ lang.get }}</span></div>
            <!--            <div class="user-phone"><span>{{ data.orderAddress.userTelCode }}</span>{{ data.orderAddress.userTel }}</div>-->
          </div>
          <div class="user-address">
            {{ data.orderAddress.countryName }}-{{ data.orderAddress.provinceName }}{{ data.orderAddress.cityName }}{{ data.orderAddress.address }}
          </div>
        ...
        <div class="send-info">
          ...
          <div class="send-infos">
            <div>{{ lang.newinfo1 }}</div>
            <div>{{ lang.newinfo2 }} {{ data.afterMail }}</div>
            <dl>
              <dt>{{ lang.newinfo3 }}</dt>
              <dd>{{ lang.newinfo4 }}</dd>
              <dd>{{ lang.newinfo5 }}</dd>
            ...
            <!--            <div v-if="data.allSend===1">{{ lang.together }}</div>-->
            <!--            <div>{{ lang.anytime }}</div>-->
            <!--            <div>-->
            <!--              {{ lang.time }}-->
            <!--              <span>-->
            <!--                <a-->
            <!--                  href="/deliveryPolicy"-->
            <!--                  target="_blank">{{ lang.policy }}</a>-->
            <!--              </span>-->
            <!--            </div>-->
            <!--            <div>{{ lang['show-ho-email'] }}<span>{{ data.afterMail }}</span></div>-->
          </div>
        </div>
        <div/>
      </div>
      <div class="right-info">
        ...
        <div class="order-infos">
          ...
          <div class="info-line">
            <div class="label">{{ lang.goodsNum }}</div>
            <div class="ff">{{ formatMoney(data.productCount, 0) }}</div>
          </div>
          <div class="info-line">
            <div class="label">{{ lang.goodsPrice }}</div>
          ...
          <div
            ...
            <div class="ff color-pink">-{{ data.coinCode }} {{ formatMoney(data.transPreferFee) }}</div>
          </div>
          <div class="big-info">
            <div>{{ lang.orderTotal }}</div>
...
import {getToken} from '@/api/cache.js';
  ...
  methods: {
    ...
    copyText() {
      try {
        ...
        this.$message.error(this.lang.msgErr);
      }
    }
  }
};
...
<style scoped lang="less">
  .finished {
    ...
    .middle-bar {
      ...
      width:1300px;
      height:30px;
      background:linear-gradient(270deg,rgba(255,255,255,1) 0%,rgba(218,146,163,1) 47%,rgba(255,255,255,1) 100%);
      line-height:30px;
      display: flex;
      align-items: center;
      justify-content: center;
